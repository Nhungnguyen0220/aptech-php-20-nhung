function createWeb() {
    ...
    nhung.onreadystatechange = function () {
      ...
      //  img.setAttribute("src", baiDauTien.image_url) 
            ...
            divCard.appendChild(divCategories);
            if(dataweb[i].categories.length >= 1){
              for( var j=0; j<dataweb[i].categories.length; j++){
                var bd=document.createElement('button');
                divCategories.appendChild(bd);
                bd.classList.add();
                bd.setAttribute('class',' badge bg-success');
                bd.innerHTML = dataweb[i].categories[j].name;
              }
            }
